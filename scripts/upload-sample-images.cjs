...
if (!supabaseUrl || !serviceKey) throw new Error('Missing Supabase URL or secret key in .env')
...
async function main() {
  for (const [title, imageUrl] of recipes) {
    ...
    if (!response.ok) {
      console.error(`Failed to download ${title}: ${response.status}`)
      continue
    }
    ...
    const { error: updateError } = await supabase.from('recipes').update({ image_url: publicUrl }).eq('title', title)
    if (updateError) {
